...
echo 「987654321@Cmscms」をOqsComAppのPWに置換　(例)0525050250@Yamada
...
echo インターネット時刻　「 ntp.nict.jp 」
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\DateTime\Servers" /v 1 /t REG_SZ /d "ntp.nict.jp" /f
...
echo　IPアドレス変更　連携用
netsh interface ipv4 set address name="連携用" source=static address=192.168.220.11 mask=255.255.255.0 gateway=192.168.220.1
netsh interface ipv4 set dns name="連携用" source=static address=8.8.8.8 register=primary validate=no
netsh interface ipv4 add dns name="連携用" address=4.4.4.4 index=2 validate=no
pause
echo.
...
echoプロキシ　例外に192.168.220.1
reg add "HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings" /f /v ProxyOverride /t reg_sz /d "*.onshikaku.org;*.flets-east.jp;*.flets-west.jp;*.lineauth.mnw;*.obn.managedpki.ne.jp;*.cybertrust.ne.jp;*.secomtrust.net;*.rece;pweb.base.oqs-pdl.org;192.168.220.1"

echo ONSRenkeiを作成
net user ONSRenkei 987654321@Cmscms /add /passwordchg:no

echo　ONSRenkeiのパスワードを無期限に
wmic useraccount where Name="ONSRenkei" set PasswordExpires=False

echo　ONSRenkeiに「administrators」を追加
net localgroup "administrators" ONSRenkei /add

echo　ONSRenkeiからuserを削除
net localgroup "users" ONSRenkei /delete

echo　OQSComappのパスワードを無期限に
wmic useraccount where Name="OQSComapp" set PasswordExpires=False

echo　オン資フォルダ作成
mkdir C:\Users\OqsComApp\Documents\オン資

echo [OQS_CRYPT_PASS] 削除
cmdkey /delete:OQS_CRYPT_PASS

echo [OQS_CRYPT_PASS] 追加
cmdkey /generic:OQS_CRYPT_PASS /user:OQS_Admin /pass:987654321@Cmscms

echo「OQS_NAS_Login_key」削除
cmdkey /delete:OQS_NAS_Login_key

echo [OQS_CRYPT_PASS] 追加
cmdkey /generic:OQS_NAS_Login_key /user:OqsComApp /pass:987654321@Cmscms

echo PC名変更
wmic computersystem where name="%computername%" call rename name="PCNAME999"

echo　oqsのPW変更
net user OqsComAPP 987654321@Cmscms
...
echo 完了しました。PC再起動してWindowsDefenderの設定をしてください
