...
#   2022/12/13  ・「OQS_CRYPT_PASS」を削除
#   2022/12/02  ・IP設定回り修正
#                   DHCP->手動設定に変更した時、以前の手動設定が残っていたら削除するように修正
#               ・「A001」の「ユーザはパスワードを変更できない」はチェックなしに
#   2022/11/11  TOSMEC対応
#               ・DNS、TVのチェックをなし
#               ・NIC設定変更
#               ・資格情報設定変更
...
#               ・プロキシ、NTP設定変更
#               ・FW無効
#   2022/10/28  TOSMEC対応+ONSRenkeiパスワード上書き修正
...
$OnDemand = New-Object $typename("&@OnDemand","@OnDemand")
$IPMembers = New-Object $typename("&IP-Members","IP-Members")
...
$Choice4 = New-Object "System.Collections.ObjectModel.Collection``1[[$assembly]]"
$Choice4.add($Ondemand)
$Choice4.add($IPMembers)
...
if($PCName.ToUpper() -cmatch "TOSMECONS01"){
    #   OK
    Write-Output ("PC名は" + $PCName + "です。")
}else{
    #   NG
    $Answer = $host.ui.PromptForChoice("<PC名が「TOSMECONS01」ではありません>","$PCName のPC名を変更しますか？",$Choice,1)
    if(-not $Answer){
        while($true){
            $NewPCName = "TOSMECONS01"
...
#   IPSEC事業者選択
#
$Answer_Vendor = $host.ui.PromptForChoice("<IPSec事業者選択>","IPSec事業者を選んで下さい",$Choice4,0)
if(-not $Answer_Vendor){
    $IPSecVendor = "@OnDemand"
    $VendorDefaultIP = "210.164.37.70"
}else{
    $IPSecVendor = "IP-Members"
    $VendorDefaultIP = "192.168.250.201"
}

Write-Output ("IPSEC事業者：" + $IPSecVendor)

#
#   ＮＩＣチェック
#
#   ※TOSMEC※
#   I211は無効、I219-LMはIP自動取得
#
...
if(-not $Answer){
    ...
    if($null -ne (Get-NetAdapter|Where-Object InterfaceDescription -Match $CoordinationAdapterName)){
        ...
        if(-not $Debug){
            ...
            #   無効だったら一度有効にする
            if((Get-NetAdapter| Where-Object InterfaceDescription -Match $ONSAdapterName).status -eq "Disabled"){
                Write-Output ("`t「オン資(v6)用」を有効にします。")
                Enable-NetAdapter -name (Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName).name -Confirm:$false
            }

            #   アダプタ名（オン資用）
            if((Get-NetAdapter| Where-Object InterfaceDescription -Match $ONSAdapterName).name -ne "オン資(v6)用"){
                #   未設定
                Write-Output ("`t名前が「オン資(v6)用」ではありません。【Fix】")
                Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Rename-NetAdapter -PassThru -ThrottleLimit 1 -ErrorAction SilentlyContinue -newname "オン資(v6)用"
            }else{
                #   設定済み
                Write-Output ("`t名前は「オン資(v6)用」です。")
            }
            ...
            if(((Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Get-NetAdapterBinding | Where-Object DisplayName -match "(TCP/IPv4)").Enabled -ne $false)){
                Write-Output ("`t「オン資(v6)用」のIPv4が有効です。【Fix】")
                ...
                Write-Output ("`t「オン資(v6)用」のIPv4は無効です。")
            }
            ...
            if(((Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Get-NetAdapterBinding | Where-Object DisplayName -match "(TCP/IPv6)").Enabled -eq $false)){
                Write-Output ("`t「オン資(v6)用」のIPv6が無効です。【Fix】")
                ...
                Write-Output ("`t「オン資(v6)用」のIPv6は有効です。")
            }
            ...
            #    Write-Output ("`t「オン資(v6)用」の自動メトリックはオフです。")
            #}else{
            #    Write-Output ("`t「オン資(v6)用」の自動メトリックはオンです。【Fix】")
            ...
            if((Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").interfacemetric[(Get-NetAdapter | Where-Object InterfaceDescription -Match "I211" | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").interfacemetric.count - 1] -eq "1"){
                Write-Output ("`t「オン資(v6)用」のメトリックは「1」です。")
            }else{
                Write-Output ("`t「オン資(v6)用」のメトリックが「1」ではありません。【Fix】")
            ...
            if((Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").dhcp[(Get-NetAdapter | Where-Object InterfaceDescription -Match "I211" | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").dhcp.count - 1] -eq "Enabled"){
                Write-Output ("`t「オン資(v6)用」のDHCPは有効です。")
            }else{
                Write-Output ("`t「オン資(v6)用」のDHCPが無効です。【Fix】")
            ...
            if((Get-NetAdapter | Where-Object InterfaceDescription -Match $ONSAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").RouterDiscovery[(Get-NetAdapter | Where-Object InterfaceDescription -Match "I211" | Get-NetIPInterface | Where-Object AddressFamily -match "IPv6").RouterDiscovery.count - 1] -eq "Enabled"){
                Write-Output ("`t「オン資(v6)用」のIPv6自動構成は有効です。")
            }else{
                Write-Output ("`t「オン資(v6)用」のIPv6自動構成が無効です。【Fix】")
            ...
            #   オン資用は無効にする
            #
            Write-Output ("`t「オン資(v6)用」を無効にします。")
            Disable-NetAdapter -Name "オン資(v6)用" -Confirm:$false
        }
        ...
        #   メトリック:1
        #
        Write-Output "オン資(v4)用(I219-LM)設定を確認します。"
        #   アダプタ名（連携用）
        if((Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName).name -ne "オン資(v4)用"){
            #   未設定
            Write-Output ("`t名前が「オン資(v4)用」ではありません。【Fix】")
            Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Rename-NetAdapter -PassThru -ThrottleLimit 1 -ErrorAction SilentlyContinue　-newname "オン資(v4)用"
        }else{
            #   設定済み
            Write-Output ("`t名前は「オン資(v4)用」です。")
        }
        ...
        if(((Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetAdapterBinding | Where-Object DisplayName -match "(TCP/IPv6)").Enabled -ne $false)){
            Write-Output ("`t「オン資(v4)用」のIPv6が有効です。【Fix】")
            ...
            Write-Output ("`t「オン資(v4)用」のIPv6は無効です。")
        }
        ...
        #   メトリック数
        if((Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4").interfacemetric[(Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4").interfacemetric.count - 1] -eq "1"){
            Write-Output ("`t「オン資(v4)用」のメトリックは「1」です。")
        }else{
            Write-Output ("`t「オン資(v4)用」のメトリックが「1」ではありません。【Fix】")
            Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | Set-NetIPInterface -InterfaceMetric 1
        }
        ...
        if((Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4").dhcp[(Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4").dhcp.count - 1] -eq "Enabled"){
            Write-Output ("`t「オン資(v4)用」のDHCPが有効です。【Fix】")
            $Answer = $host.ui.PromptForChoice("「オン資(v4)用」のIPv4アドレスは自動取得になっています。",$IPSecVendor + "用に修正しますか？",$Choice,1)
			if(-not $Answer){
                #   DHCPをオフ
                ...
                switch ($IPSecVendor) {
                    "@OnDemand" {
				        #   IPアドレス設定
                        Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | New-NetIPAddress -AddressFamily IPv4 -IPAddress 210.164.37.70 -PrefixLength 28 -DefaultGateway 210.164.37.65
                        #   DNS設定
                        Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | Set-DnsClientServerAddress -ServerAddresses 210.164.37.65

                    }
                    "IP-Members" {
				        #   IPアドレス設定
                        Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | New-NetIPAddress -AddressFamily IPv4 -IPAddress 192.168.250.201 -PrefixLength 24 -DefaultGateway 192.168.250.1
                        #   DNS設定
                        Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | Set-DnsClientServerAddress -ServerAddresses 192.168.30.101,192.168.30.102

                    }
                    Default {}
                }
                Write-Output ("`t「オン資(v4)用」のIP設定を「" + $IPSecVendor + "」用に修正しました【Fix】")
            }
        }else{
            Write-Output ("`t「オン資(v4)用」のDHCPは無効です。")
        }
        ...
        Write-Output ("`t「オン資(v4)用」の現在の設定")
        ...
        if(($CheckIPAddress.IPAddress -ne $VendorDefaultIP)){
            $Answer = $host.ui.PromptForChoice("「オン資(v4)用」のIPv4アドレスが「" + $IPSecVendor + "」用と異なります。","再設定しますか？",$Choice,1)
            if(-not $Answer){
                #   再設定
                #   DHCPをオフ
				Get-NetAdapter | Where-Object InterfaceDescription -Match $CoordinationAdapterName | Get-NetIPInterface | Where-Object AddressFamily -match "IPv4" | Where-Object CompartmentId -ne $null | Set-NetIPInterface -dhcp Disabled
                ...
                Write-Output ("`t「オン資(v4)用」のIP設定(" + $IPSecVendor + ")を再設定しました【Fix】")
...
$CheckAccount = 'OQSComApp','ONSRenkei',"A001"
    ...
    if ((IsLocalUserAccunt $Str_CA) -eq $false){
        ...
        switch ($Str_CA) {
            ...
            'A001'{
                ...
                #   ユーザはパスワードを変更できない
                #New-LocalUser -Name $Str_CA -Password (ConvertTo-SecureString $SecPass -AsPlainText -Force) -PasswordNeverExpires　-UserMayNotChangePassword   
                New-LocalUser -Name $Str_CA -NoPassword  -AccountNeverExpires
...
foreach($Chk_SN in $Check_Sharename){
    ...
    #   A001アクセス権がない
    if ($Check_A001 -eq $false){
        Write-output ("`t" + $Chk_SN + "への、「A001」のアクセス権がありません。作成します。【Fix】")
        Grant-SmbShareAccess -Name $Chk_SN -AccountName A001 -AccessRight Full -Force
    }
...
foreach ($TargetFolder in $Check_Folder) {
    ...
    switch ($TargetFolder) {
        "C:\OQS" {
            $Check_Account = "Authenticated Users"
        }
        "C:\ProgramData\OQS\OQSComApp\work\output"{
            $Check_Account = "ONSRenkei","A001"
        }
        Default {
            #   あり得ない
    ...
    if(test-path $TargetFolder){
        foreach ($Target_Account in $Check_Account){
...
if(test-path($RegPath)){
    ...
    if($null -eq $ChkReg.LimitBlankPasswordUse){
        ...
        if ( ($ChkReg.LimitBlankPasswordUse -eq 1)){
            Write-Output ("`t「空のパスワードの使用をコンソールログオンのみに制限する」が有効です。無効にします。【Fix】")
...
foreach ($FWLocation in $Target_FW_Location) {
    ...
    if ((Get-NetFirewallProfile -Profile $FWLocation).Enabled){
        Write-Output ("`t「" + $FWLocation + "」のFWが有効です。")
        Set-NetFirewallProfile -Profile $FWLocation -Enabled False
        Write-Output ("`t「" + $FWLocation + "」のFWを無効にしました。【Fix】")
    }else{
        Write-Output ("`t「" + $FWLocation + "」のFWは無効です。") 
    }
...
#	環境切替ツール確認
#
$ChangeEnvironmentLatestVersion = "v1.0.4"
$ChangeEnvironmentInstallFile = "ChangeEnvironment_v1.0.4.zip"
...
if(($Result -contains '* NONE *') -or ($Result -contains '* なし *')){
    #   資格情報なし、追加
    write-host "「OQS_CRYPT_PASS」は存在しません。"
    #cmdkey /generic:OQS_CRYPT_PASS /user:OQS_Admin /pass:$CryptPass
    #cmdkey /generic:OQS_CRYPT_PASS /user:OQS_Admin
    ...
    #}
    write-host "「OQS_CRYPT_PASS」が存在します。削除します【Fix】"
    cmdkey /delete:OQS_CRYPT_PASS
}
...
if(($Result -contains '* NONE *') -or ($Result -contains '* なし *')){
    ...
    cmdkey /generic:OQS_NAS_LOGIN_KEY /user:A001
}else{
...
#   プロキシ確認
#
#   TOSMEC  @Ondemand用定義追加(www.odvpn)

write-output "--"
Write-Output ("プロキシ設定を確認します。(" + $IPSecVendor + "用)")
...
if(($Proxy_Result -match '直接アクセス') -or ($Proxy_Result -match 'Direct access')){
    ...
    switch ($IPSecVendor) {
        "@OnDemand" {
            #   プロキシ設定
            netsh winhttp set proxy proxy-server="proxy.healthcare-on-demand.jp:8080" bypass-list="*.onshikaku.org;*.flets-east.jp;*.flets-west.jp;*.lineauth.mnw;*.obn.managedpki.ne.jp;*.cybertrust.ne.jp;*.secomtrust.net;*.rece;pweb.base.oqs-pdl.org;www.odvpn"
        }

        "IP-Members" {
            #   プロキシ設定
            netsh winhttp set proxy proxy-server="proxy.base.oqs-pdl.org:8080" bypass-list="*.onshikaku.org;*.flets-east.jp;*.flets-west.jp;*.lineauth.mnw;*.obn.managedpki.ne.jp;*.cybertrust.ne.jp;*.secomtrust.net;*.rece;pweb.base.oqs-pdl.org"
...
if(test-path($RegPath)){
    ...
    switch ($IPSecVendor) {
        "@OnDemand" {
            #   プロキシ設定
            Set-ItemProperty -path $RegPath -name ProxyServer -value 'proxy.healthcare-on-demand.jp:8080'
            Set-ItemProperty -path $RegPath -name ProxyOverride -value '*.onshikaku.org;*.flets-east.jp;*.flets-west.jp;*.lineauth.mnw;*.obn.managedpki.ne.jp;*.cybertrust.ne.jp;*.secomtrust.net;*.rece;pweb.base.oqs-pdl.org;www.odvpn'
        }
    
        "IP-Members" {
            #   プロキシ設定
            Set-ItemProperty -path $RegPath -name ProxyServer -value 'proxy.base.oqs-pdl.org:8080'
            Set-ItemProperty -path $RegPath -name ProxyOverride -value '*.onshikaku.org;*.flets-east.jp;*.flets-west.jp;*.lineauth.mnw;*.obn.managedpki.ne.jp;*.cybertrust.ne.jp;*.secomtrust.net;*.rece;pweb.base.oqs-pdl.org'
        }
    }
}else{
    Write-Host ("レジストリエントリ" + $RegPath + "がありません。【エラー】") -ForegroundColor "Red"
...
#
#   NTPサーバ設定(TOSMEC @Ondemand用)
#
write-output "--"
Write-Output ("NTPサーバを設定します。(" + $IPSecVendor + "用)")

switch ($IPSecVendor) {
    "@OnDemand" {
        #   NTP設定
        Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\w32time\Parameters" -Name "NtpServer" -Value "ntp.healthcare-on-demand.jp,0x9"
        set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\DateTime\Servers" -Name "0" -Value "ntp.healthcare-on-demand.jp"
    ...
    "IP-Members" {
        #   NTP設定
        Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\w32time\Parameters" -Name "NtpServer" -Value "ntp.base.oqs-pdl.org,0x9"
        set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\DateTime\Servers" -Name "0" -Value "ntp.base.oqs-pdl.org"
        set-ItemProperty -path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\DateTime\Servers" -Name "(default)" -Value "0"
